export default async function handler(req, res) {
  try {
    ...
    const { idea } = req.body;
    if (!idea) {
      return res.status(400).setHeader('Access-Control-Allow-Origin', '*').setHeader('Access-Control-Allow-Methods', 'POST, OPTIONS').json({ error: 'Idea is required' });
    }

    const systemPrompt = `Generate a detailed prompt for Midjourney/DALL-E using the idea: ${idea}. The prompt should be creative, unique, and descriptive. Use a tone that is imaginative and artistic. Provide a prompt that can be used to create a unique art piece.`;
    const messages = [
